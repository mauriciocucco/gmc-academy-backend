...
- `GET /api/v1/admin/students/:id` devuelve `id`, `fullName`, `email`, `phone`, `profilePhotoUrl`, `approved`, `blocked`, `blockedAt`, `blockReason`, `lastAttemptScore`, `note`, `progress` y `stats`; `note` devuelve `{ content, updatedAt, updatedByName }` o `null`; `approved` refleja el ultimo intento y `progress.examPassed` refleja si el alumno aprobo al menos un examen.
- `POST /api/v1/auth/login`, `POST /api/v1/auth/refresh` y `GET /api/v1/me` incluyen `mustChangePassword` dentro del usuario autenticado. Si `users.blocked_at` no es `null`, `POST /api/v1/auth/login`, `POST /api/v1/auth/refresh` y cualquier ruta protegida por `JwtAuthGuard` responden `403 ForbiddenException`. `PATCH /api/v1/me/password` recibe `{ currentPassword, newPassword }`, limpia `mustChangePassword` y borra el `refresh_token_hash` previo.
- `PATCH /api/v1/admin/students/access` recibe `{ studentIds: string[], blocked: boolean, reason?: string }` y devuelve `{ items: [{ id, blocked, blockedAt, blockReason }] }`; rechaza con `404` si algun `studentId` no existe o no pertenece a un alumno.
